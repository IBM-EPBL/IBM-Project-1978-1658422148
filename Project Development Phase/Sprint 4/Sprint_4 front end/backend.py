...
def login():
    ...
    total = [[float(temp), float(do), float(ph), float(
        co), float(bod), float(na), float(tc), int(year)]]
    payload_scoring = {"input_data": [{"field": [
        ["temp", "do", "ph", "co", "bod", "na", "tc", "year"]], "values": total}]}
    response_scoring = requests.post('https://us-south.ml.cloud.ibm.com/ml/v4/deployments/b61b8352-d24c-4ede-b0f4-9dcd50fffa28/predictions?version=2022-11-19',
